...
def measure_dac_step(daq, idac, step, chipid, mode):
    ...
    dac=DACS[idac]
    dac_addr=0x601+idac
    adc_input=5 if dac_addr<0x60A else 6
    ...
    if mode:
        for k, sel_input in [['Value', adc_input], ['AVDD', 2]]:
...
daq.fw_reset()
daq.alpide_cmd_issue(0xD2)
daq.fw_clear_monitoring()
daq.alpide_cmd_issue(0xE4)

if args.via_on_chip_adc:
    daq.alpide_setreg_adc_ctrl(Mode=2,
                               SelInput=0,
                               SetIComp=2,
                               DiscriSign=0,
                               RampSpd=1,
...
try:
    for idac in tqdm(range(len(DACS)), desc="Overall progress"):
        if DACS[idac] == 'VCASN':
            for step in tqdm(range(55, 60), leave=False, desc="scan: %s" % (DACS[idac])):
